...
interface TextNormalProps {
  ...
  elipsis?: 'head' | 'middle' | 'tail' | 'clip';
}

const TextNormal: React.FC<TextNormalProps> = props => {
  const {
    title,
    color,
    backgroundColor,
  ...
  return (
    <Text numberOfLines={numberOfLines} ellipsizeMode={elipsis} allowFontScaling={false} style={textStyle}>
      {title}
    </Text>
  );
};

export default memo(TextNormal);
